...
def seg_postprocessing(pred_masks, data):
    ...
    probs_onehotNcombine = OnehotNcombine( resize_probs ) # probs = pred_mask에 softmax 씌운 것 (output size 3차원 --> B x H x W)           
    probs_onehotNcombine = np.array(probs_onehotNcombine.squeeze(0).cpu())

    return probs_onehotNcombine

def save_images(webpage, visuals, image_path, data, aspect_ratio=1.0, width=256):
    ...
    slice_num  = data['slice_num'][0]
    new_image_dir = os.path.join(image_dir, patient_id)
    new_name = name + patient_id + slice_num
    ...
    for label, im_data in visuals.items():
        ...
        image_name = '%s_%s.nii.gz' % (new_name, label)
        save_path = os.path.join(new_image_dir, image_name)
        if 'mask' in image_name:
            im = seg_postprocessing(im_data, data)
        try:
